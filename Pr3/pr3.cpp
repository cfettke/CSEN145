...
csr_t matmultCSR(const csr_t* A, const csr_t* B) {
    if (A->ncols != B->nrows) {
        std::cerr << "Wrong dimensions\n";
        exit(1);
    }
    ...
    C->reserve(A->rows, 1);
    C->ind = new idx_t[A->nrows * B->ncols]; 
    C->val = new val_t[A->nrows * B->ncols];
    C->ptr = new ptr_t[A->nrows + 1];

    ptr_t nnz_count = 0;  // Non-zero count for result matrix

    //row A
    for (idx_t i = 0; i < A->nrows; ++i) {
        C->ptr[i] = nnz_count;  // Set pointer for the current row in result
        //column B 
        for (idx_t j = 0; j < B->ncols; ++j) {
            val_t dotProduct = 0;
            //calcs dot product
            for (ptr_t k = A_>ptr[i]; k < A->ptr[i + 1]; ++k) {
                idx_t colA = A->ind[k];
                val_t valA = A->val[k];
                //Finds elemnts in b
                for (ptr_t l = B->ptr[colA]; l < B->ptr[colA + 1]; ++l) {
                    idx_t colB = B->ind[l];
                    val_t valB = B->val[l];
            ...
            if (dotProduct != 0) {
                C->ind[nnz_count] = j;
                C->val[nnz_count] = dotProduct;
                nnz_count++;
            }
        }
    }

    C->ptr[A.nrows] = nnz_count;  
